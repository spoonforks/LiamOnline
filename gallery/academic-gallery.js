...
const THESIS_PDF_URL = encodeURI("Academic Works/Thesis/AI4G181.pdf");
...
thesisFrameImage.src = "Academic Works/Thesis/directionalhallu.png";
...
parametricAnimationSheetImage.src = "CanopyEvolution-spritesheet.webp";
...
// The software renderer is the compatibility baseline. It avoids a black
// viewport on browsers that lose or reject a nested-iframe WebGL context.
const webglRenderer = null;
...
function addFace(collection, points, fill, stroke = "rgba(74, 63, 50, 0.08)", sortBias = 0) {
  const face = { points, fill, stroke, sortBias, isMesh: false };
...
function addBenchModel(collection, obstacleCollection) {
  ...
  for (const triangle of benchModelData.triangles) {
    ...
    const benchFace = addFace(
      collection,
      [a, b, c],
      shadeColor(baseColor, lightAmount),
      "rgba(0, 0, 0, 0)"
    );
    benchFace.isMesh = true;
  }
...
function render() {
  ...
  const meshProjectionLimit = Math.max(state.width, state.height) * 6;
  ...
  for (const face of faces) {
    ...
    const points = clipped.map(projectPoint);
    if (points.some((point) => !Number.isFinite(point.x) || !Number.isFinite(point.y))) continue;
    if (face.isMesh && points.some((point) => Math.abs(point.x) > meshProjectionLimit || Math.abs(point.y) > meshProjectionLimit)) {
      continue;
    }

    const depths = clipped.map((point) => point.z);
    const depth = face.isMesh
      ? Math.max(...depths)
      : depths.reduce((total, value) => total + value, 0) / depths.length;
    projectedFaces.push({
      fill: face.fill,
      stroke: face.stroke,
      depth: depth + (face.sortBias || 0),
      points
    });
  }
  ...
  for (const sprite of sprites) {
    const cameraPoints = sprite.points.map(toCameraSpace);
    if (cameraPoints.some((point) => point.z <= nearPlane)) continue;
...
window.addEventListener("wheel", (event) => {
  if (state.hasEnteredGallery || event.ctrlKey || window.parent === window) {
    return;
  }

  event.preventDefault();
  notifyHost("gallery:wheel", { deltaY: event.deltaY });
}, { passive: false });
